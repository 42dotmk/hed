...
static inline int vec_reserve_typed(void *vec_ptr, size_t new_cap, size_t elem_size) {
    ...
    if (!vec || new_cap <= vec->cap) return 1;
...
#define vec_push(vec, type, value) do { \
    if (!vec_ensure_cap((Vector*)(vec))) break; \
...
#define vec_remove(vec, type, index) do { \
    if ((index) < (vec)->len) { \
