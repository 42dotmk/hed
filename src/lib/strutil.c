...
static size_t utf8_decode_char(const unsigned char *p, size_t remaining, wchar_t *out_wc) {
    ...
    if ((c & 0xF0) == 0xE0) {
        ...
        *out_wc = (wchar_t)(((c & 0x0F) << 12) | ((p[1] & 0x3F) << 6) | (p[2] & 0x3F));
        return 3;
    }
    ...
    if ((c & 0xF8) == 0xF0) {
        ...
        *out_wc = (wchar_t)(((c & 0x07) << 18) | ((p[1] & 0x3F) << 12) | ((p[2] & 0x3F) << 6) | (p[3] & 0x3F));
        return 4;
    }
...
void utf8_slice_by_columns(const char *str, size_t byte_len,
    ...
    if (!str || !out_byte_start || !out_byte_len) {
        if (out_byte_start) *out_byte_start = 0;
