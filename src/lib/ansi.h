 ...
 * + background without touching reverse video. Used by the renderer
 * when closing a syntax span inside a selection so the selection's
 * inverse-video stays on (49 is safe there — inverse swaps the default
 * bg in, it doesn't rely on a bg attribute; 27 must stay out or a
 * span close inside a selection would kill the highlight). Must clear
 * every attribute a span can set (the tasks plugin's CANCELLED status
 * uses SGR 9, the completion menu's selection bar sets a background),
 * else the attribute leaks past the span and bleeds into the rest of
 * the frame and following buffers. */
static inline void ansi_sgr_soft_reset(Abuf *ab) {
    ab_append(ab, "\x1b[22;23;24;29;39;49m", 20);
}
