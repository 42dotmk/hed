...
const MouseEvent *ed_last_mouse(void) { return &g_last_mouse; }
        ...
        if (read(fd, &seq[0], 1) != 1) {
            ...
            if (seq[1] == '<') {
                ...
                if (parse_ok && ni == 2 && (term == 'M' || term == 'm')) {
                    key = mouse_decode(nums[0], nums[1], nums[2], term == 'm');
                } else {
                    key = '\x1b';
                }
                ...
                if (!parse_ok) {
                     ...
                     * comes from `n`. */
                    char mod_b, term;
                    if (read(fd, &mod_b, 1) != 1 || read(fd, &term, 1) != 1) {
                        key = '\x1b';
                    } else {
                        int b = 0;
                        if (n == 1) {
                        ...
                        if (b) {
                            ...
                            case '3':
                                ...
                                break;
                            case '4':
                                flags = KEY_SHIFT | KEY_META;
                                break;
                            case '5':
                                flags = KEY_CTRL;
                                break;
                            case '6':
                                flags = KEY_SHIFT | KEY_CTRL;
                                break;
                            case '7':
                                flags = KEY_META | KEY_CTRL;
                                break;
                            case '8':
                                flags = KEY_SHIFT | KEY_META | KEY_CTRL;
                                break;
                            }
                            key = flags | b;
                        } else {
