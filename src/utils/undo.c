...
static void undo_vec_push(UndoVec *v, const UndoRec *r) {
    undo_vec_reserve(v, v->len + 1);
    ...
    while (i <= len) {
        if (i == len || data[i] == '\n') {
            ...
            if (i < len && data[i] == '\n') {
                ...
                buf_row_insert_in(buf, y + 1, rest, rest_len);
...
int undo_perform(void) {
    ...
    for (int i = start; i < UNDO.len; i++) {
        undo_vec_push(&REDO, &UNDO.items[i]);
        used_bytes -= UNDO.items[i].payload.len;
    }
    UNDO.len = start;
    applying = 0;
...
int redo_perform(void) {
    ...
    /* move to UNDO */
    for (int i = start; i < REDO.len; i++) {
        undo_vec_push(&UNDO, &REDO.items[i]);
