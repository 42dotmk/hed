...
static void hist_insert_front(CmdHistory *h, const char *line) {
    if (!line || !*line) return;
    if (h->len + 1 > h->cap) {
        int ncap = h->cap == 0 ? 64 : h->cap * 2;
        char **n = realloc(h->items, ncap * sizeof(char*));
        if (!n) return;
        h->items = n; h->cap = ncap;
    }
    memmove(&h->items[1], &h->items[0], sizeof(char*) * h->len);
    h->items[0] = strdup(line);
    h->len++;
    if (h->len > CMD_HISTORY_MAX) {
        free(h->items[CMD_HISTORY_MAX]);
        h->len = CMD_HISTORY_MAX;
    }
}

static void hist_append(CmdHistory *h, const char *line) {
    if (!line || !*line) return;
    if (h->len >= CMD_HISTORY_MAX) return;
    if (h->len + 1 > h->cap) {
        int ncap = h->cap == 0 ? 64 : h->cap * 2;
        char **n = realloc(h->items, ncap * sizeof(char*));
        if (!n) return;
...
static void hist_prepend_to_file(const char *line) {
    ...
    if (old && oldsz > 0) {
        int lines_kept = 0;
        const char *p = old; const char *end = old + oldsz;
        while (p < end && lines_kept < CMD_HISTORY_MAX - 1) {
            const char *nl = memchr(p, '\n', (size_t)(end - p));
            if (!nl) { keep_bytes = oldsz; lines_kept++; break; }
    ...
    if (!buf) { free(old); free(path); return; }
    memcpy(buf, line, llen);
    buf[llen] = '\n';
    if (keep_bytes) memcpy(buf + llen + 1, old, keep_bytes);
    buf[newsz] = '\0';
    ...
    if (tfp) {
        ...
        tfp = fopen(path, "w");
        if (tfp) { fwrite(buf, 1, newsz, tfp); fclose(tfp); }
    }
    free(buf); free(old); free(path);
}

void hist_init(CmdHistory *h) {
    if (!h) return;
    h->items = NULL; h->len = 0; h->cap = 0; h->idx = -1;
    h->saved_len = 0; h->prefix_len = 0;
    h->saved_line[0] = '\0'; h->prefix[0] = '\0';
    ...
    while ((r = getline(&line, &cap, fp)) != -1) {
        ...
        line[r] = '\0';
        hist_append(h, line);
        if (h->len >= CMD_HISTORY_MAX) break;
    }
    free(line); fclose(fp); free(path);
}
...
void hist_reset_browse(CmdHistory *h) {
    if (!h) return;
    h->idx = -1;
    h->saved_len = 0; h->saved_line[0] = '\0';
    h->prefix_len = 0; h->prefix[0] = '\0';
}

static int hist_prefix_match(const char *entry, const char *pre, int plen) {
    if (!entry) return 0;
    for (int i = 0; i < plen; i++) {
        if (!entry[i] || entry[i] != pre[i]) return 0;
    }
    return 1;
}
...
int hist_browse_up(CmdHistory *h, const char *current_input, int current_len,
    ...
    if (!h || h->len == 0 || out_cap <= 0) return 0;
    if (h->idx == -1) {
        h->saved_len = current_len;
        if (h->saved_len > (int)sizeof(h->saved_line) - 1) h->saved_len = (int)sizeof(h->saved_line) - 1;
        memcpy(h->saved_line, current_input, h->saved_len);
        h->saved_line[h->saved_len] = '\0';
        h->prefix_len = current_len;
        if (h->prefix_len > (int)sizeof(h->prefix) - 1) h->prefix_len = (int)sizeof(h->prefix) - 1;
...
int hist_browse_down(CmdHistory *h, char *out, int out_cap, int *restored) {
    ...
    int n = h->saved_len;
    if (n > out_cap - 1) n = out_cap - 1;
    memcpy(out, h->saved_line, n); out[n] = '\0';
    if (restored) *restored = 1;
    return 1;
}

int hist_len(const CmdHistory *h) { return h ? h->len : 0; }
const char *hist_get(const CmdHistory *h, int idx) {
    if (!h || idx < 0 || idx >= h->len) return NULL;
    return h->items[idx];
}
