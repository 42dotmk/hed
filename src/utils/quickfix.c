...
static void qf_update_window_view(Qf *qf) {
    ...
    if (buf->num_rows > 0) {
        for (int i = 0; i < buf->num_rows; i++) {
            Row *row = &buf->rows[i];
            if (!row->chars.data || row->chars.len == 0) continue;
            char desired = (i == sel) ? '*' : ' ';
    ...
    for (int wi = 0; wi < (int)E.windows.len; wi++) {
        ...
        if (w->buffer_index == buf_index && w->is_quickfix) {
            ...
            w->cursor.x = 0;
            if (w->height > 0) {
                if (cursor_row < w->row_offset) w->row_offset = cursor_row;
                else if (cursor_row >= w->row_offset + w->height) w->row_offset = cursor_row - w->height + 1;
...
static int qf_pick_target_window_index(void) {
    ...
    /* Try to find a non-quickfix window above the quickfix pane, overlapping horizontally. */
    int qf_top = cur->top;
    ...
    for (int i = 0; i < (int)E.windows.len; i++) {
        ...
        if (w_bottom < qf_top &&
            !(w_right < qf_left || w_left > qf_right)) {
    ...
    /* No non-quickfix window exists: create one by splitting the quickfix window.
     * We split horizontally and treat the original window (cur_idx) as the one
     * above, and the new window as the quickfix pane below. */
    int prev_idx = cur_idx;
    windows_split_horizontal();
    if (!BOUNDS_CHECK(prev_idx, E.windows.len)) return prev_idx;

    int new_idx = (int)E.windows.len - 1;
    if (!BOUNDS_CHECK(new_idx, E.windows.len)) return prev_idx;
...
static void qf_jump_to_internal(const QfItem *it, int focus_target) {
    ...
    if (it->filename && it->filename[0]) {
        /* Reuse existing buffer for this filename if present; otherwise open it. */
