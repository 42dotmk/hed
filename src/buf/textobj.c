...
static int clamp_line(const Buffer *buf, int line) {
    if (!buf || buf->num_rows <= 0) return -1;
    if (line < 0) line = 0;
...
static int set_selection(TextSelection *sel, TextPos start, TextPos end, TextPos cursor) {
    if (!sel) return 0;
    /* Ensure start <= end */
    if (start.line > end.line || (start.line == end.line && start.col > end.col)) {
        TextPos tmp = start; start = end; end = tmp;
    }
    sel->start = start;
    sel->end = end;
    sel->cursor = cursor;
...
static int word_range_at(const Row *row, int col, int *sx, int *ex) {
    if (!row || row->chars.len == 0 || !sx || !ex) return 0;
...
static int paragraph_range(Buffer *buf, int line, int *out_sy, int *out_ey) {
    if (!buf || buf->num_rows == 0 || !out_sy || !out_ey) return 0;
...
static int is_unescaped_quote(const Row *row, int x) {
    if (!row || x < 0 || x >= (int)row->chars.len) return 0;
    int backslashes = 0;
    for (int i = x - 1; i >= 0 && row->chars.data[i] == '\\'; i--) backslashes++;
    return (backslashes % 2) == 0;
}

static int find_enclosing_pair(Buffer *buf, int line, int col,
                               char open, char close,
                               int *oy, int *ox, int *cy, int *cx) {
    if (!buf || buf->num_rows == 0 || !oy || !ox || !cy || !cx) return 0;
    int cur_y = clamp_line(buf, line);
    if (cur_y < 0) return 0;
    const Row *cur_row = &buf->rows[cur_y];
    if (cur_row->chars.len == 0) return 0;
...
static int brackets_select(Buffer *buf, int line, int col, char open, char close, int include_delims, TextSelection *sel) {
    int oy, ox, cy, cx;
    if (!find_enclosing_pair(buf, line, col, open, close, &oy, &ox, &cy, &cx)) return 0;

    if (include_delims) {
        TextPos start = { oy, ox };
        TextPos end = { cy, cx + 1 };
        TextPos cursor = { clamp_line(buf, line), clamp_col(&buf->rows[clamp_line(buf, line)], col) };
...
int textobj_word(Buffer *buf, int line, int col, TextSelection *sel) {
    ...
    int sx = 0, ex = 0;
    if (!word_range_at(row, x, &sx, &ex)) return 0;
    TextPos cursor = { y, x };
...
int textobj_brackets(Buffer *buf, int line, int col, TextSelection *sel) {
    ...
    if (y < 0) return 0;
    Row *row = &buf->rows[y];
    if (!row || row->chars.len == 0) return 0;
    int x = clamp_col(row, col);
    int probe = x;
    if (probe >= (int)row->chars.len) probe = (int)row->chars.len - 1;
...
int textobj_brackets_with(Buffer *buf, int line, int col, char open, char close, int include_delims, TextSelection *sel) {
    if (!open || !close) return 0;
...
int textobj_to_word_end(Buffer *buf, int line, int col, TextSelection *sel) {
    ...
    if (!word_range_at(row, x, &sx, &ex)) return 0;
    int start_col = x;
    if (start_col < sx) start_col = sx;
    if (start_col > ex) start_col = ex;
    TextPos cursor = { y, x };
    return set_selection(sel, (TextPos){ y, start_col }, (TextPos){ y, ex }, cursor);
}
...
int textobj_to_word_start(Buffer *buf, int line, int col, TextSelection *sel) {
    ...
    if (!word_range_at(row, x, &sx, &ex)) return 0;
    int end_col = x;
    if (end_col < sx) end_col = sx;
    if (end_col > ex) end_col = ex;
    TextPos cursor = { y, x };
...
int textobj_to_file_end(Buffer *buf, int line, int col, TextSelection *sel) {
    ...
    TextPos cursor = { y, x };
    return set_selection(sel, (TextPos){ y, x }, (TextPos){ last_y, last_len }, cursor);
}
...
int textobj_to_paragraph_end(Buffer *buf, int line, int col, TextSelection *sel) {
    int y = clamp_line(buf, line);
    if (y < 0) return 0;
    int sy = 0, ey = 0;
    if (!paragraph_range(buf, y, &sy, &ey)) return 0;
    Row *row = &buf->rows[y];
    int x = clamp_col(row, col);
...
int textobj_to_paragraph_start(Buffer *buf, int line, int col, TextSelection *sel) {
    int y = clamp_line(buf, line);
    if (y < 0) return 0;
    int sy = 0, ey = 0;
    if (!paragraph_range(buf, y, &sy, &ey)) return 0;
