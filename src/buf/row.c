...
int buf_row_cx_to_rx(const Row *row, int cx) {
    ...
    for (int i = 0; i < cx;) {
        ...
        int adv = 1;
        if ((c & 0x80) == 0) adv = 1;            /* ASCII */
        else if ((c & 0xE0) == 0xC0) adv = 2;    /* 2-byte */
        else if ((c & 0xF0) == 0xE0) adv = 3;    /* 3-byte */
        else if ((c & 0xF8) == 0xF0) adv = 4;    /* 4-byte */
        if (i + adv > cx) adv = cx - i; /* don't cross boundary */
        i += adv;
