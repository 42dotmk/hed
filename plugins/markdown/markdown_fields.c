...
static const MdFieldDef FIELDS[] = {
    ...
    {"archived", MD_FK_DATE},  {"prio", MD_FK_PRIO},
    {"priority", MD_FK_PRIO},  {"tags", MD_FK_TAGS},
    {"owner", MD_FK_TEXT},     {"assignee", MD_FK_TEXT},
    {"id", MD_FK_TEXT},
};
...
int md_field_is_key(const Row *row, const char *key) {
    int k0, k1, v0, v1;
    if (!md_parse_field(row->chars.data, (int)row->chars.len, &k0, &k1, &v0,
                        &v1))
        return 0;
    size_t kl = strlen(key);
    return (int)kl == k1 - k0 && strncmp(row->chars.data + k0, key, kl) == 0;
}
