...
static const LspServerDef SERVERS[] = {
    {"c", ARGV("clangd"),
     ROOTS("compile_commands.json", ".clangd", "Makefile", ".git")},
    {"cpp", ARGV("clangd"),
     ROOTS("compile_commands.json", ".clangd", "Makefile", ".git")},
    {"rust", ARGV("rust-analyzer"), ROOTS("Cargo.toml", ".git")},
    {"python", ARGV("pyright-langserver", "--stdio"),
     ROOTS("pyproject.toml", "setup.py", "requirements.txt", ".git")},
    {"go", ARGV("gopls"), ROOTS("go.mod", ".git")},
    {"typescript", ARGV("typescript-language-server", "--stdio"),
     ROOTS("tsconfig.json", "package.json", ".git")},
    {"javascript", ARGV("typescript-language-server", "--stdio"),
     ROOTS("tsconfig.json", "package.json", ".git")},
...
const LspServerDef *lsp_servers_lookup(const char *lang) {
    ...
    for (int i = 0; i < n; i++) {
        if (strcmp(SERVERS[i].lang, lang) == 0)
            return &SERVERS[i];
    }
    return NULL;
}
