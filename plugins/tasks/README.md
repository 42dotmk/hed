...
<space>X    :task_capture ▏       (global, not markdown-only)
```
...
`[<subject>](mail://thread:…)`, and `gf` on it reopens that thread —
mail buffers are temporary, but the thread id is stable.
