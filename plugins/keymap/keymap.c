...
static void apply(const char *name) {
    ...
    if (strcmp(name, "vim") == 0) {
        if (plugin_vim_keybinds.init)
            plugin_vim_keybinds.init();
        ed_set_modeless(0);
        g_active = "vim";
    } else if (strcmp(name, "emacs") == 0) {
        if (plugin_emacs_keybinds.init)
            plugin_emacs_keybinds.init();
        ed_set_modeless(1);
        g_active = "emacs";
    } else if (strcmp(name, "vscode") == 0) {
        if (plugin_vscode_keybinds.init)
            plugin_vscode_keybinds.init();
        ed_set_modeless(1);
        g_active = "vscode";
    } else {
        ed_set_status_message("unknown keymap: %s (try vim|emacs|vscode)",
                              name);
        return;
    }
...
static void cmd_keymap_toggle(const char *args) {
    (void)args;
    if (strcmp(g_active, "vim") == 0)
        apply("emacs");
    else if (strcmp(g_active, "emacs") == 0)
        apply("vscode");
    else
        apply("vim");
}
...
static int keymap_init(void) {
    ...
    cmd("keymap-toggle", cmd_keymap_toggle,
        "cycle keymap (vim → emacs → vscode)");
    return 0;
}
