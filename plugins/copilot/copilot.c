...
static const char *cp_language_id(Buffer *buf) {
    ...
    if (!buf || !buf->filetype || !*buf->filetype)
        return "plaintext";
    return buf->filetype;
}
...
static void cp_pane_refresh(void) {
    ...
    for (int i = 0; i < CP.alts_count; i++) {
        const char *disp =
            CP.alts[i].display ? CP.alts[i].display : CP.alts[i].text;
        if (!disp)
            disp = "";
        const char *p = disp;
        int line_no = 0;
...
static void cp_clear_suggestion(void) {
    if (!CP.has_suggestion && CP.alts_count == 0)
        return;
    for (ptrdiff_t i = 0; i < arrlen(E.buffers); i++) {
        vtext_clear_ns(&E.buffers[i], CP.vt_ns);
    }
    free(CP.suggestion_text);
    CP.suggestion_text = NULL;
    free(CP.suggestion_display);
    CP.suggestion_display = NULL;
    free(CP.suggestion_uuid);
...
static void cp_dismiss(void) {
    if (CP.suggestion_uuid)
        cp_notify_rejected(CP.suggestion_uuid);
    cp_clear_suggestion();
}
...
static void cp_request_completions_for_cursor(Buffer *buf) {
    if (!CP.spawned || !CP.initialized || !CP.signed_in)
        return;
    if (!buf || !buf->filename || !buf->cursor)
        return;
    if (cp_is_pane_buf(buf))
        return; /* never request for our own pane */
...
static void cp_debounced_fire(void *ud) {
    ...
    if (E.mode != MODE_INSERT)
        return;
    if (!cp_cursor_at_eol(buf))
        return; /* v1: EOL only */
    cp_request_completions_for_cursor(buf);
}

static void cp_schedule(void) {
    if (!g_enabled || !CP.spawned || !CP.initialized || !CP.signed_in)
        return;
    ed_loop_timer_after("copilot:debounce", CP_DEBOUNCE_MS, cp_debounced_fire,
                        NULL);
}
...
static void cp_send_did_open(Buffer *buf) {
    if (!buf || !buf->filename || !CP.spawned || !CP.initialized)
        return;
    if (cp_is_pane_buf(buf))
        return; /* never sync our own pane */
    char *uri = cp_uri_for(buf->filename);
    char *text = buf_to_text(buf, NULL);
    if (!uri || !text) {
        free(uri);
...
static void on_buffer_close(HookBufferEvent *e) {
    if (!e || !e->buf || !CP.spawned || !CP.initialized)
        return;
    if (!e->buf->filename)
        return;
    if (cp_is_pane_buf(e->buf))
        return;
...
static void cp_show_active(int line, int col) {
    ...
    if (nl && *(nl + 1)) {
        ...
        while (tail_len > 0 && tail[tail_len - 1] == '\n')
            tail_len--;
        if (tail_len > 0)
            vtext_set_block_below(buf, CP.vt_ns, line, tail, tail_len, NULL);
    }
    if (show_len <= 0 && !(nl && *(nl + 1)))
        return;
...
static void cp_handle_sign_in_initiate(cJSON *result) {
    ...
    const char *uri =
        result ? json_get_string(result, "verificationUri") : NULL;
    if (!code || !uri) {
    ...
    ed_set_status_message(
        "copilot: open %s and enter code %s, then :copilot login %s", uri, code,
        code);
...
void cp_handle_message(const char *json_str, int len) {
    ...
    if (!json) {
        log_msg("copilot: JSON parse error");
        return;
    }
...
static void kb_accept(void) {
    ...
    if (row >= 0 && row < buf->num_rows) {
        ...
        if (max > flen)
            max = flen;
        while (strip < max && line[strip] == full[strip])
            strip++;
        if (strip < max)
            strip = 0; /* prefixes diverged — insert all */
    }
...
static int copilot_init(void) {
    ...
    if (CP.vt_ns >= 0)
        vtext_ns_set_auto_clear(CP.vt_ns, 0);
