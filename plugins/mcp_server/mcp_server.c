...
static cJSON *tool_read_current_buffer(void) {
    ...
    if (!out)
        return tool_result_text("error: out of memory", 1);
...
static cJSON *tool_apply_edit(cJSON *args) {
    ...
    char msg[128];
    snprintf(msg, sizeof(msg), "ok: replaced %d lines [%d,%d) with %d lines",
             end - start, start, end, inserted);
    ed_set_status_message("mcp: apply_edit [%d,%d) -> %d lines", start, end,
                          inserted);
...
static cJSON *tool_run_command(cJSON *args) {
    ...
    while (*line == ' ' || *line == '\t' || *line == ':')
        line++;
    if (!*line)
        return tool_result_text("error: empty cmdline", 1);
...
static void drain_client(int fd, void *ud) {
    ...
    if (c->rx_len + (size_t)n + 1 > c->rx_cap) {
        ...
        while (ncap < c->rx_len + (size_t)n + 1)
            ncap *= 2;
        if (ncap > MCP_RX_LIMIT) {
            client_close(c);
            return;
        }
...
static int mcp_start(void) {
    ...
    snprintf(g_socket_path, sizeof(g_socket_path), "/tmp/hed-mcp-%d.sock",
             (int)getpid());
...
static void cmd_mcp_status(const char *args) {
    ...
    for (int i = 0; i < MCP_MAX_CLIENTS; i++)
        if (g_clients[i].fd > 0)
            n++;
    ed_set_status_message("mcp: %s (%d client%s)", g_socket_path, n,
                          n == 1 ? "" : "s");
}
...
static void mcp_deinit(void) { mcp_stop(); }
